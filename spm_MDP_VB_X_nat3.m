function [MDP, list, inf_length, KL] = spm_MDP_VB_X_nat3(MDP,OPTIONS)
...
% maximise negative expected free energy: expected free energy is the free energy of
...
% minimises the KL divergence between the posterior predictive distribution
...
%--------------------------------------------------------------------------
if size(MDP,2) > 1 %size(MDP,2) =number of trials
                    %size(MDP,1) =number of agents
    ...
    for i = 1:size(MDP,2) %loop over number of trials
        for m = 1:size(MDP,1) %loop over number of agents
            
            % update concentration parameters
            %--------------------------------------------------------------
            if i > 1
                try  MDP(m,i).a = OUT(m,i - 1).a; end %try to give it the parameters of the last trial (i.e. here passing on Dirichlet parameters)
                try  MDP(m,i).b = OUT(m,i - 1).b; end %this only works if we are doing inference over Dirichlet parameters (i.e. they are present)
        ...
        % solve this trial (for all models synchronously) i.e. for all
        % agents simultaneously
        %------------------------------------------------------------------
        [OUT(:,i), list{i}, inf_length(i), KL(i)] = spm_MDP_VB_X_nat3(MDP(:,i),OPTIONS); %USE caution: so far the information length only works when the MDP only has one agent
...
%for Hidden Markov models, data analysis 
for m = 1:size(MDP,1)
...
Ni    = 512;                                % number of VB iterations
for m = 1:size(MDP,1)
    ...
    for f = 1:Nf(m)
        for j = 1:Nu(m,f)
            ...
            if isfield(MDP,'b') && ~HMM
                ...
                sB{m,f}(:,:,j) = spm_norm(MDP(m).B{f}(:,:,j) );
                rB{m,f}(:,:,j) = spm_norm(MDP(m).B{f}(:,:,j)'); %rB is the normalised transpose of sB
            end
...
% only relevant for multiple agents
%--------------------------------------------------------------------------
for m = 1:size(MDP,1)
    n      = -MDP(m).o;
    N(m,:) = mode(n.*(n > 0),1);
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if ~HMM % not required for HMM
            ...
            for g = 1:Ng(m)
                ...
                if MDP(m).o(g,t) < 0
                    ...
                    % most likely outcome is realised, based upon the A
                    % matrix MDP(m).A{g}
                    %only for multiagent games
                    %------------------------------------------------------
                    n = -MDP(m).o(g,t);
                    MDP(m).n(g,t) = n;
                    if n == m
                ...
                elseif MDP(m).o(g,t) == 0
                    ...
                    %generate the outcomes from the generative process
                    %------------------------------------------------------
        ...
        % Only for hierarchical models (link is what specifies hierarchy)
        %==================================================================
        if isfield(MDP,'link') 
            ...
            %--------------------------------------------------------------
            try
                mdp = MDP(m).mdp(t); %mdp as a subfield of MDP is the lower level
            catch
                mdp = MDP(m).MDP;
            end
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_nat3(mdp); %calls itself recursively
        ...
        if isfield(MDP,'demi')
            ...
            %spm_MDP_DEM is the continuous state-space routine
            %spm_ADEM is the one that is called inside that is horrible
            for g = 1:Ng(m)
        ...
        %==================================================================
        if ~HMM || T == t %HMM= Hidden Markov model
            ...
            if ~isfield(MDP,'U') && t > 1
                ...
                %occam's window over policies: tree search
            end
            
            % processing time and reset
            % make beliefs a bit less precise (to avoid values at the boundary to be treated as zero)
            %--------------------------------------------------------------
            tstart = tic;
            for f = 1:Nf(m)
            ...
            F     = zeros(Np(m),1); % initialise value of free-energy for each policy 
            for k = p{m}                % loop over plausible policies
                ...
                for i = 1:Ni            % iterate belief updates
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        for f = 1:Nf(m) %iterate over hidden state factors
                            ...
                            %----------------------------------------------
                            if dF > exp(-8) || i <= 4 %if the change in free-energy is greater than a threshold (otherwise minimum is attained)
                            %or if i>4.
                                ...
                                if j <= t
                                    ...
                                    %f means omitting the f hidden state
                                    %factor in the marginals
                                    qL = spm_log(qL(:));
                                end
                                
                                % (negative) entropy
                                %------------------------------------------
                                qx  = spm_log(sx); %categorical parameters on the log simplex
                                
                                % empirical priors (forward messages)
                                % messages coming from the past
                                %------------------------------------------
                                if j < 2
                                    px = spm_log(D{m,f});
                                    v  = v + px + qL - qx;
                                ...
                                if j < R
                                    ...
                                    %rB is the normalised transpose of sB
                                    v  = v + px + qL - qx;
                                end
                                
                                % (negative) free energy
                                %------------------------------------------
                                F(k) = F(k) + sx'*v/Nf(m); %dot product between v, sx divided by the number of state factors
                                %gets from the gradient back to the free
                                %energy
                                %the expectation of the gradient is taken
                                %with respect to sx
                                
                                % update
                                %------------------------------------------
                                eap = v;
                                v    = v - mean(v); %gradient of the negative free-energy | mean(v) is always ~0 -- substract it to remove numerical errors.
                                %the sum of the components =0 ensures that
                                %the result remains on the simplex (since simplex => sum of components = 1)
                                %HOWEVER THIS IS MADE REDUNDANT BY THE USE
                                %OF SOFTMAX IN Karl's UPDATE

                               list{t,m,k,i,j,f}(1,:)=sx; %store sx before update to then be able to compute information length                                
                               
                                %parameter update for the categorical distribution using gradient ascent on negative free-energy
                                %qx are the parameters on the log-simplex,
                                %sx are the parameters on the simplex                      
                                
                                sx   = sx + sx.*eap/tau; %natural gradient update if: well defined <=> resulting sx has strictly positive components.
                                if sum(sx) > 0
                                    sx   = sx/sum(sx); % check that this indeed corresponds to a projection
                                end
                                    
                                if sum(sx <=0) > 0
                                    sx   = spm_softmax(qx + v/tau); %Karl's gradient update
                                end

                               list{t,m,k,i,j,f}(2,:)=sx; %store sx after update to then be able to compute information length
                                
                                %sx = spm_softmax(qx + diag(exp(-qx)/max(exp(-qx)))*v/tau);          
                            else
                                F(k) = G(k);
                            end
                            
                            % store update neuronal activity
                            %largely stored for plotting purposes
                            %----------------------------------------------
                            x{m,f}(:,j,k)      = sx;
                            xq{m,f}            = sx;
                            xn{m,f}(i,:,j,t,k) = sx;
                    ...
                    % convergence
                    %------------------------------------------------------
                    if i > 1 %i number of belief updates (16)
                        dF = F(k) - G(k); %=sx'*v/Nf(m) scores the change in free energy with respect to the last iteration? Something like that
                    end
                    G = F;
            ...
            % accumulate expected free energy of policies (Q is G)
            % depends on our belief about states, preferences 
            %==============================================================
            ...
            if Np(m) > 1
                for k = p{m}
                    ...
                    if isfield(MDP,'d')
                        for f = 1:Nf(m)
                            ...
                            %second term is salience or novelty
                        end
                    end
                ...
                for i = 1:Ni
                    ...
                    if OPTIONS.gamma
                        ...
                        dFdg    = qb{m} - beta + eg;
%                         if qb{m}*dFdg < tau
%                           qb{m}   = qb{m} - qb{m}^2*dFdg/tau; % natural gradient if result is well-defined. i.e. qb{m}>0
%                         elseif dFdg < tau*qb{m}
                          qb{m}   = qb{m} - dFdg/tau; %normal gradient update if result is well-defined.
%                         else
%                           qb{m}   = qb{m}/2; %updates but ensures that qb{m} remains positive
%                         end  
                        w{m}(t) = 1/qb{m}; %expectation of gamma = 1/qb{m}. qb{m}=rate of Gamma distribution
                    end
            ...
            %--------------------------------------------------------------
            MDP(m).F(:,t) = F; %free energy for each policy
            MDP(m).G(:,t) = Q; %expected free energy for each policy
            MDP(m).H(1,t) = qu'*MDP(m).F(p{m},t) - qu'*(log(qu) - log(pu));
            %free energy taking all policies into account
    ...
    end % end of loop over models (agents) m = M(t,:)
...
% compute total information length for categorical parameters
%==========================================================================
inf_length=0;
KL=0;
% for t = 1:T                 %loop over time points
%     for m = M(t,:)              %loop over number of agents
%         for k = p{m}                % loop over plausible policies
%             for i = 1:Ni            	% iterate belief updates (Ni = 16 typically)
%                 for j = 1:S                 % loop over future time points 
%                     for f = 1:Nf(m)             %loop over hidden state factors
%                     try inf_length = inf_length + categorical_inf_length(list{t,m,k,i,j,f}(1,:), list{t,m,k,i,j,f}(2,:)); end  
%                     %try KL = KL + KL_length(list{t,m,k,i,j,f}(1,:), list{t,m,k,i,j,f}(2,:)); end
%                     end
%                 end
%             end
%         end
...
for m = 1:size(MDP,1)
    ...
    for t = 1:T         %timestep                                                    %timesteps
        
        % mapping from hidden states to outcomes: a 
        %------------------------------------------------------------------
        if isfield(MDP,'a')                                                 %check whether a is a field of the MDP object
            for g = 1:Ng(m)                                                 %Ng(m) = number of outcome factors
                da     = O{m}(g,t);
                for  f = 1:Nf(m)
                    da = spm_cross(da,X{m,f}(:,t));
                end
                MDP(m).a{g} = MDP(m).a{g} + eta*da.*(MDP(m).a{g} > 0);%no inference on the parameters of a which are zero
%                 av = MDP(m).a{g}(:);
%                 av2=av;
%                 indexes = av<=0;
%                 dav = da(:);
%                 dav(indexes)=[];
%                 av(indexes)=[];
%                 fim = diag(psi(1,av))-psi(1,sum(av)); 
%                 if det(fim) ~= 0
%                     av2(~indexes)= av + eta*pinv(fim)*dav.*diag(fim); %NATURAL GRADIENT UPDATE
%                     MDP(m).a{g} = reshape(av2,size(MDP(m).a{g}));
%                 else
%                     %MDP(m).a{g} = MDP(m).a{g} + eta*da.*(MDP(m).a{g} > 0); %update in direction of fixed point
%                     av2(~indexes)= av + eta*dav.*diag(fim); %Normal gradient update
%                     MDP(m).a{g} = reshape(av2,size(MDP(m).a{g}));
%                 end
            end
        end
...
%{
function psivalue = newpsi(number)
    if number >0
        psivalue = psi(1,number);
    elseif number ==0
        psivalue = 0;
    else 
        error('newpsi argument is negative')
    end
%}
