...
% plot performance with and without received wisdom
%--------------------------------------------------------------------------
...
% with and without natural gradient
%==========================================================================
%BMR.g   = 1;
%BMR.f   = 2;
%BMR.T   = 3;
...
Ns    = 128; %number of subjects
for m = 1:Ns
    
    % create structure array and solve
    %----------------------------------------------------------------------
    clear MDP NatDP
    for i = 1:N %create an array of MDPs, corresponding to the number of trials to be executed by each agent
        MDP(i) = mda; %mda is an MDP
        NatDP(i) = mda;
    end
...
%     subplot(4,1,1)
%     b = bar(mean(inf_length2(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 0 1]);%, hold off
%     %b = bar(mean(R(:,:),2)); set(b,'EdgeColor','w','FaceColor',[1 1 1]*.0), hold off
%     xlabel('trial'), ylabel('Avg. Inf. length');%, axis([1/2 (N + 1/2) 1/3 1]);
%     title('Karl','Fontsize',16)
%     
%     subplot(4,1,2)
%     b = bar(mean(inf_length(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 1 0]);%, hold off
%     %b = bar(mean(R(:,:),2)); set(b,'EdgeColor','w','FaceColor',[1 1 1]*.0), hold off
%     xlabel('trial'), ylabel('Avg. Inf. length');%, axis([1/2 (N + 1/2) 1/3 1]);
%     title('Natural gradient','Fontsize',16)
%     
%     subplot(4,1,3)
%     b = bar(mean(inf_length(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 1 0]), hold on
%     b = bar(mean(inf_length2(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 0 1]), hold off
%     xlabel('trial'), ylabel('Avg. Inf. length');%, axis([1/2 (N + 1/2) 1/3 1]);
%     title('Comparison','Fontsize',16)
%     
%     subplot(4,1,4)
%     b = bar(mean(inf_length2(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 0 1]), hold on
%     b = bar(mean(inf_length(:,:),1)); set(b,'EdgeColor','w','FaceColor',[0 1 0]), hold off
%     xlabel('trial'), ylabel('Avg. Inf. length');%, axis([1/2 (N + 1/2) 1/3 1]);
%     title('Comparison','Fontsize',16)
    ...
    title('Active inference','Fontsize',16) %the opposite
    ylim([0 16])
    ...
    %b = bar(mean(R(:,:),2)); set(b,'EdgeColor','w','FaceColor',[1 1 1]*.0), hold off
    xlabel('trial'), ylabel('Information length');%, axis([1/2 (N + 1/2) 1/3 1]);
    title('Natural gradient','Fontsize',16) %the opposite
...
%     %-----------------------------------
      ...
      subplot(2,1,1)
      boxplot(mean(nat_length(:,:),1)-mean(inf_length(:,:),1))
      ylabel('Active inference - Natural gradient'); % the opposite
      title('Performance difference','Fontsize',16)
      
      subplot(2,1,2)
      boxplot(abs(mean(nat_length(:,:),1)-mean(inf_length(:,:),1)))
